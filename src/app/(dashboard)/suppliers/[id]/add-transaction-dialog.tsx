...
const CASH_SOURCES = [
  { id: "cash_office", name: "دفع نقدي من المكتب" },
  { id: "cash_warehouse", name: "دفع نقدي من المخزن" },
];

// For suppliers:
//   debit  → purchase / invoice from supplier (supplier delivered goods)
//   credit → payment we made to supplier
//   manual → adjustment
...
export function AddSupplierTransactionDialog({
  open, onOpenChange, supplierId, userId, banks, supplierName,
}: AddSupplierTransactionDialogProps) {
  ...
  async function onSubmit(values: FormValues) {
    ...
    if (values.type === "debit") {
      // Purchase from supplier: qty × price → debit (we owe supplier)
      ...
      // Manual adjustment: positive = supplier owes us more (credit),
      //                    negative = we owe supplier more (debit)
      const amt = Number(values.amount) || 0;
      if (amt >= 0) { credit = amt; }
      else { debit = Math.abs(amt); }
      if (credit === 0 && debit === 0) { toast.error("يجب إدخال مبلغ التعديل"); return; }
      if (!description) { toast.error("التفاصيل مطلوبة"); return; }
    }

    const isBankSource = values.payment_source && !CASH_SOURCES.some((s) => s.id === values.payment_source);

    const { error } = await safeInsert("supplier_transactions", {
    ...
    // If payment via bank, also create bank transaction (money OUT of bank).
    if (credit > 0 && isBankSource) {
      ...
      if (bankError) {
        toast.error("تم حفظ قيد المورد لكن فشل تسجيل العملية البنكية");
        return;
      }
    }
    ...
    form.reset({
      entry_date: todayISO(), type: "debit", description: "",
      quantity: "", price: "", amount: "", payment_source: "",
    });
    onOpenChange(false);
    router.refresh();
  }
      ...
      <DialogContent className="max-w-md">
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
            ...
            <FormField control={form.control} name="type" render={({ field }) => (
              <FormItem>
                ...
                <Select onValueChange={field.onChange} value={field.value}>
                  ...
                  <SelectContent>
                    <SelectItem value="debit">عليه (مشتريات / فاتورة)</SelectItem>
                    <SelectItem value="credit">له (دفع للمورد)</SelectItem>
              ...
              <>
                <FormField control={form.control} name="description" render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl><Input {...field} placeholder="مثال: شحنة عادة مصريين" /></FormControl>
                    <FormMessage />
                  </FormItem>
                )} />
                <div className="grid grid-cols-2 gap-4">
                  <FormField control={form.control} name="quantity" render={({ field }) => (
                    <FormItem>
                      <FormLabel>العدد / الكمية</FormLabel>
                      <FormControl><Input type="number" step="0.001" {...field} dir="ltr" className="text-left" /></FormControl>
                      <FormMessage />
                    </FormItem>
                  )} />
                  <FormField control={form.control} name="price" render={({ field }) => (
                    <FormItem>
                      ...
                      <FormControl><Input type="number" step="0.01" {...field} dir="ltr" className="text-left" /></FormControl>
                      <FormMessage />
                    </FormItem>
                  )} />
                </div>
            ...
            {/* === CREDIT (Payment to supplier) === */}
            {txType === "credit" && (
              <>
                <FormField control={form.control} name="amount" render={({ field }) => (
                  <FormItem>
                    <FormLabel>المبلغ المدفوع</FormLabel>
                    <FormControl><Input type="number" step="0.01" min="0" {...field} dir="ltr" className="text-left" /></FormControl>
                    <FormMessage />
                  </FormItem>
                )} />
            ...
            {txType === "manual" && (
              <>
                <FormField control={form.control} name="description" render={({ field }) => (
                  <FormItem>
                    <FormLabel>التفاصيل</FormLabel>
                    <FormControl><Input {...field} placeholder="مثال: تسوية رصيد" /></FormControl>
                    <FormMessage />
                  </FormItem>
                )} />
                <FormField control={form.control} name="amount" render={({ field }) => (
                  <FormItem>
                    <FormLabel>المبلغ (موجب = له، سالب = عليه)</FormLabel>
                    <FormControl><Input type="number" step="0.01" {...field} dir="ltr" className="text-left" /></FormControl>
                    <FormMessage />
                  </FormItem>
                )} />
